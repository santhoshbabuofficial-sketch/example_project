...
bool CurrentSensor::Read(float& current_a_out) {                   // Reads ACS712 current value.
    ...
    LOG_INF("ACS712 raw=%u Vpin=%d.%02u mV Iout=%d.%02u A", raw_code,
            static_cast<int>(adc_pin_voltage),
            static_cast<unsigned>((adc_pin_voltage - static_cast<int>(adc_pin_voltage)) * 100),
            static_cast<int>(current_a_out),
            static_cast<unsigned>((current_a_out - static_cast<int>(current_a_out)) * 100));
    // Prints raw code and computed current for debugging.
