...
#include <zephyr/kernel.h>                                          // Added by Santhosh: brings in k_busy_wait() for the direction-reversal dead-time.
...
void MotorControl::Tick() {                                                                    // Advances the ramp by one step.
    ...
    if (ramp_state_ == RampState::kIncreasing) {                                                  // Handles the accelerate phase.
        ...
        if (pwm_percent_ <= kPwmStepPercent) {                                                                // Checks if this step reaches zero.
            ...
            // Added by Santhosh: dead-time gap so the outgoing IR2104 leg is
            // fully off (gate charge/discharge, driver propagation delay)
            // before the incoming leg is enabled, preventing shoot-through
            // across the H-bridge on direction reversal.
            k_busy_wait(kDirectionDeadTimeUs);
