...
int main() {                                                        // Main function starts the PWM ramp up/down task.
    ...
    while (true) {                                                            // Repeats the ramp up / hold / ramp down cycle forever.
        ...
        for (uint8_t duty_percent = kPwmMinPercent;                              // Ramps duty upward one step at a time.
             duty_percent <= kPwmMaxPercent;
             duty_percent += kPwmStepPercent) {
            ...
            k_msleep(kRampStepPeriodMs);                                            // Waits before applying the next ramp-up step.
        }
        ...
        for (int16_t duty_percent = kPwmMaxPercent;                             // Ramps duty downward one step at a time.
            ...
            k_msleep(kRampStepPeriodMs);                                            // Waits before applying the next ramp-down step.
        }
    }
