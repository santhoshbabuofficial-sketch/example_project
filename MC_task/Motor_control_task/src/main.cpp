...
using mc::motor_control::CurrentSensor;                          // Creates short name for current sensor class.
using mc::motor_control::FaultDetector;                          // Creates short name for fault detector class.
using mc::motor_control::FaultReason;                            // Creates short name for fault reason enum.
using mc::motor_control::GpioOverlay;                             // Creates short name for GPIO overlay class.
using mc::motor_control::LcdDisplay;                              // Creates short name for LCD display class.
using mc::motor_control::LcdSensorObserver;                       // Creates short name for LCD observer class.
using mc::motor_control::MotorControl;                            // Creates short name for motor control class.
using mc::motor_control::SensorManager;                           // Creates short name for sensor manager class.
using mc::motor_control::VoltageSensor;                           // Creates short name for voltage sensor class.


constexpr uint16_t kSensorSamplePeriodMs = 1000U;                 // Defines sensor reading period (also fault-check period).
constexpr uint16_t kRampStepPeriodMs = 300U;                      // Defines how often the PWM ramp advances by one 5% step.
...
int64_t g_last_button_press_ms = 0;                                    // Stores timestamp of the last accepted button press.
...
void ButtonWorkHandler(k_work* /*work*/) {                              // Runs in thread context when a button press is processed.
...
void ButtonIsr(const device* /*port*/, gpio_callback* /*cb*/, uint32_t /*pins*/) {
// Runs in interrupt context when the user button is pressed.

    const int64_t now_ms = k_uptime_get();                                 // Reads current uptime in milliseconds.

    if ((now_ms - g_last_button_press_ms) < kButtonDebounceMs) {            // Checks debounce window.

        return;                                                              // Ignores presses that are too close together.
    }

    g_last_button_press_ms = now_ms;                                         // Updates last accepted press timestamp.
...
void RampThreadEntry(void* /*p1*/, void* /*p2*/, void* /*p3*/) {
    ...
    while (true) {                                                             // Creates infinite RTOS task loop.
        ...
        if (!g_motor->is_faulted()) {                                            // Skips LCD update while a fault message is shown.
            ...
            g_lcd->ShowPwmStatus(g_motor->pwm_percent(),
                                  g_motor->direction() == mc::motor_control::Direction::kForward);
