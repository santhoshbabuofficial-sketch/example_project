...
// --- Plain GPIO outputs ---------------------------------------------------
constexpr gpio_pin_t kGpioPin2 = 2U;                               // PC2.
constexpr gpio_pin_t kGpioPin3 = 3U;                               // PC3.

// --- Motor characteristics -----------------------------------------------
constexpr uint32_t kMotorRatedRpm = 100U;                          // Motor shaft speed at 100% duty (no gearbox - this is the actual output speed).

// --- What this run should do --------------------------------------------
constexpr uint32_t kTargetDegrees = 45U;                          // Output-shaft angle to turn per step.
constexpr uint32_t kDrivePercent = 50U;                           // PWM duty (= assumed % of rated speed) used while turning.
constexpr uint32_t kRepeatCount = 3U;                             // Number of 45-degree steps to run.
...
int main() {                                                        // Main function starts the PWM task.
    ...
    const device* gpioc_dev = DEVICE_DT_GET(DT_NODELABEL(gpioc));          // Gets the GPIOC port device (already enabled in the board overlay).

    if (!device_is_ready(gpioc_dev)) {                                     // Checks that GPIOC initialized correctly.

        LOG_ERR("GPIOC device not ready - halting");                          // Prints GPIO device error.

        return -1;                                                            // Stops program due to hardware failure.
    }

    const int pc2_cfg_err = gpio_pin_configure(gpioc_dev, kGpioPin2, GPIO_OUTPUT_ACTIVE); // Configures PC2 as an output and immediately drives it high.
    ...
    const int pc3_cfg_err = gpio_pin_configure(gpioc_dev, kGpioPin3, GPIO_OUTPUT_ACTIVE); // Configures PC3 as an output and immediately drives it high.
